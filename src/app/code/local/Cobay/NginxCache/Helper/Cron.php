<?php
class Cobay_NginxCache_Helper_Cron extends Mage_Core_Helper_Abstract {
	 ...
	 * @var string
	 */
	const CRAWLER_URLS_CACHE_ID = 'nginxcache_crawler_url_queue';
	...
	public function addUrlsToCrawlerQueue(array $urls) {
		...
		if ($this->getCrawlerDebugEnabled()) {
			foreach ($urls as $url) {
				Mage::helper('nginxcache/debug')->log('Adding URL to queue: %s', $url );
			}
		}
	...
	public function getCrawlerClient() {
		if (is_null($this->_crawlerClient)) {
			$this->_crawlerClient = new Varien_Http_Client(null, array(
				'useragent'     => sprintf(
					'Cobay_NginxCache/%s Magento/%s Varien_Http_Client',
					Mage::helper('nginxcache/data')->getVersion(),
					Mage::getVersion() ),
				'keepalive'     => true,
			));
	...
	public function getCrawlerEnabled() {
		return Mage::getStoreConfig('system/nginxcache/crawler_enable');
	}
	...
	public function getCrawlerDebugEnabled() {
		return Mage::getStoreConfig('system/nginxcache/crawler_debug');
	}
	...
	public function getCrawlerBatchSize() {
		return Mage::getStoreConfig('system/nginxcache/crawler_batchsize');
	}
	...
	public function getCrawlerWaitPeriod() {
		return Mage::getStoreConfig('system/nginxcache/crawler_batchwait');
	}
