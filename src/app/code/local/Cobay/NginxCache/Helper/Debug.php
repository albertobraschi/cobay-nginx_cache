<?php
class Cobay_NginxCache_Helper_Debug extends Mage_Core_Helper_Abstract {
	/**
	 * Logs errors
	 *
	 * @param $message
	...
	{
		/*
		if ( ! Mage::helper('nginxcache/varnish')->getVarnishDebugEnabled()) {
			return;
		}
		*/
	...
	public function dump($value) {
		Mage::register('nginxcache_nocache_flag', true, true);
		$this->logValue($value);
		echo '<pre>'.PHP_EOL;
		var_dump($value);
		echo '</pre>'.PHP_EOL;
	...
	public function logBackTrace($backTrace = null) {
		...
		$btuuid = Mage::helper('nginxcache/data')->generateUuid();
	...
	public function useCustomLogFile() {
		return Mage::getStoreConfigFlag(
			'system/nginxcache/use_custom_log_file' );
	}
	...
	public function getCustomLogFileName() {
		return (string) Mage::getStoreConfig(
			'system/nginxcache/custom_log_file_name' );
	}
